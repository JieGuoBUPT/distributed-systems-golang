...
func (kv *KVPaxos) Get(args *GetArgs, reply *GetReply) error {
	...
	if handled {
		if args.Seq == kv.client_info[args.ClientId].highest_seq { // has already processed
			...
			kv.Log("Get request was a duplicate, returning original reply")
			return nil
		} else {
			kv.Log("Get request has already been processed and is not most recent!")
			return nil
		}
	}
	...
	kv.Log("Finished Get request %d!", instance_num, op)
	return nil
}

func (kv *KVPaxos) doGet(op *Op) {
	kv.Log("Performing Get operation for %s\n", op.GetArgs.Key)
...
func (kv *KVPaxos) PutAppend(args *PutAppendArgs, reply *PutAppendReply) error {
	...
	if handled {
		if args.Seq == kv.client_info[args.ClientId].highest_seq { // has already processed
			...
			kv.Log("PutAppend was duplicate, returning original reply")
			return nil
		} else {
			err := fmt.Errorf("PutAppend (%v) has already been processed, but is not most recent!", args)
			// log.Panicf(err)
			return err
		}
	}
	...
	kv.Log("Finished PutAppend request %d!", instance_num)
	return nil
}

func (kv *KVPaxos) doPutAppend(op *Op) {
	kv.Log("Applying PutAppend operation. (%d %d) to %s\n", op.ClientId, op.Seq, op.PutAppendArgs.Key)
...
func (kv *KVPaxos) doOp(op *Op, seq int) {
	...
	if handled {
		kv.Log("Already handled Op with ClientId %d, Seq %d (%d, %d) PaxosSeq %d!", op.ClientId, op.Seq, handled_op.ClientId, handled_op.Seq, handled_op.PaxosSeq)
	} else {
		switch op.OpType {
		case GetOp:
			kv.doGet(op)
